...
RUN echo 'Maike' | vncpasswd -f > $HOME/.vnc/passwd
...
RUN echo 'whoami ' >>/Maike.sh

RUN echo 'cd ' >>/Maike.sh

RUN echo "su -l -c 'vncserver :2000 -geometry 1360x768' "  >>/Maike.sh

RUN echo 'cd /noVNC-1.2.0' >>/Maike.sh

RUN echo './utils/launch.sh  --vnc localhost:7900 --listen 8900 ' >>/Maike.sh

RUN chmod 755 /Maike.sh
